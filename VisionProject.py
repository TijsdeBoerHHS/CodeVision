...
def getbluecoord(frame):
    ...
    blue_contours, blue_hierarchy = cv2.findContours(blue_mask.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
def getredcoord(frame):
    ...
    red_contours, red_hierarchy = cv2.findContours(red_mask.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
def getyellowcoord(frame):
    ...
    yellow_contours, yellow_hierarchy = cv2.findContours(yellow_mask.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    output_yellow = cv2.drawContours(frame, yellow_contours, -1, (0, 255, 255), 3)
...
def getgreencoord(frame):
    ...
    green_contours, green_hierarchy = cv2.findContours(green_mask.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
def getorangecoord(frame):
    ...
    orange_contours, orange_hierarchy = cv2.findContours(orange_mask.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    output_orange = cv2.drawContours(frame, orange_contours, -1, (0, 165, 255), 3)
...
def getwhitecoord(frame):
    ...
    white_contours, white_hierarchy = cv2.findContours(white_mask.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    output_white = cv2.drawContours(frame, white_contours, -1, (0, 165, 255), 3)
    if len(white_contours):
        M = cv2.moments(white_contours[0])   
        if M['m00'] != 0:
            cx = int(M['m10']/M['m00'])
            cy = int(M['m01']/M['m00'])
            cv2.drawContours(frame, [white_contours[0]], -1, (255, 255, 255), 2)
            cv2.circle(frame, (cx, cy), 7, (0, 0, 255), -1)
            cv2.putText(frame, "center", (cx - 20, cy - 20),
            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2)
            print(f"x: {cx} y: {cy}")
...
def getpurplecoord(frame):
    ...
    purple_contours, purple_hierarchy = cv2.findContours(purple_mask.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    output_purple = cv2.drawContours(frame, purple_contours, -1, (0, 165, 255), 3)
...
def getnextframe(colors,frame):
    ...
    if all(items is False for items in colors) == False:
        if detect == False:
            print("Piece Error")
            return(frame,0,0,"PE",0) # PE stands for Piece Error, which means no needed pieces found
